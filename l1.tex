...
Da die exakte Berechnung einer Kollision zwischen zwei Objekten $o_0, o_1 \in \obj$ signifikant viel Rechenleistung benötigt, ist eine Vorfilterung der Möglichen Objektpaare sinnvoll, die die meisten Paare schon vorher ausschließt.\\
...
Für bestimmte Zwecke, meist logische Kollisionen (vgl. \ref{sec:usages}), ist diese abstrakte Kollision zwischen Hüllobjekten sogar schon ausreichend, um eine Reaktion einzuleiten.\\
...
In jeder Kategorie
%GRR auf was bezieht sich "Kategorie"? (symetrisch/asymetrisch), oder innerhalb, können mehrere Interaktionstypen
%GRR wie zufor was ist ein Interaktionstyp? wie ist der definiert? spezifizität
 enthalten sein. Jeder Typ
%GRR ***
 wird hier einzeln betrachtet und ihm kann so ein eigener Vorfilterungsalgorithmus zugewiesen werden, der entsprechend zugeschnitten und problemspezifisch optimiert werden kann.
Die Aufgabe des Vorfilterungsalgorithmus unterscheidet sich je nach Kategorie
%GRR definition schon voher benötigt
 der Interaktion etwas:
\begin{enumerate}
\item Symmetrische Interaktionen: Aus einer Menge von Objekten $\obj$ werden Objektpaare ausgewählt, deren AABBs sich überschneiden
(ein beliebiges der beiden Objekte muss dann über die Interaktion mit dem Partner informiert werden
%GRR ich glaube das ist ein implementierungsdetail, das hier nur entweder nicht verständlich oder zu umständlich dargestellt werden kann
). 
...
\item Asymmetrische Interaktionen: Mit 2 Mengen $\obj_M, \obj_S$ wird als Master- und Slavemenge werden alle Objektpaare mit jeweils einem Mitglied aus jeder Menge ausgewählt, deren AABBs sich überschneiden 
%GRR master slave terminoligie der Mengen und Interpretationen unbedingt gefordert
% es entstehen später diskrepanzen in der semantik von aussagen
% u.U. nicht konform mit Änderungen => Diskussion
(das Objekt der Master-Klasse muss dann über seine Inkteraktion mit dem Partner aus der Slave-Klasse informiert werden
%GRR ???????? was ? warum ? das hier überhaupt wichtig?
). 
Als untere Schranke kann man $\Omega(|\obj_M|+|\obj_S|+|C|)$ angeben. Die obere Schranke $\mathcal{O}(|\obj_M|+|\obj_S|)$ erhält man durch naives Ausprobieren jeder Kombination.
\end{enumerate}
...
%GRR Wer sagt das ? Wenn nur du das sagst, sollte das begründet werden!
...
%GRR Query-Box so schreiben oder eindeutschen
...
Ein Knoten referenziert genau eine einsortierte AABB. Als Sortierschlüssel wird dort das niedrige Extremum der einsortierten AABB in einer bestimmten Dimension, $x_{min},y_{min}$ oder $z_{min}$  (Definition vgl. \ref{sec:aabb}) gewählt, es gilt also 
//TODO Mathe
%GRR todo
 key aus S. 
Ist das obere Ende der Query-Box in dieser Dimension kleiner als der Schlüssel, kann die höhere
%GRR definiere terminologie "hoch" und "niedrig" in relation zu Baumstruktur
 Baumhälfte ausgeschlossen werden und muss nicht mehr durchsucht werden. \\
Die Sortierdimension ist hier für jeden Knoten individuell wählbar.\\
% Ist das die einzige Option? Nein. Wieso wurde diese gewählt? Was macht das besonders? Was hofft man damit zu erreichen/vermeiden?
Um die untere Baumhälfte ausschließen zu können, müssen jedoch weitere Informationen vorliegen.

 Da die maximale Ausdehnung
%GRR von was ? eines Knotens/einer AABB ? Welcher ?
 nach oben in der vorher festgelegten Koordinate
%GRR welche Koordinate? das ist die erste Erwähnung von Koordinate
 nicht durch den Knoten festgelegt werden kann, muss jeder Knoten, der darunter liegt, berücksichtigt werden und der Maximalwert
%GRR Maximalwert von was? Aller unterliegenden Knoten?
  aller ermittelt werden. Dieser wird nach vollständigem Baumaufbau dann im Knoten vermerkt.\\
%GRR Dieser Satz war sehr hart bis gar nicht verständlich, zu unspezifisch. Mach es mir einfacher!
%GRR Würde das hier auch anders aufbauen
% Zuerst erklären was vorliegt, ruhig wiederholen, um relevante sachen in den Vordergrund zu bringen
% "Im Knoten ist das und das definiert, bzw. nicht definiert,liegen jene Informationen vor /nicht vor "
%GRR an dieser Stelle würde sich langsam eine Listung,Klassendiagramm oder Grafik für einen Knoten lohnen
% dann das Problem daran schildern
% "daher kann die und die Aussage nicht getroffen werden"
Wird zu beiden Teilbäumen auch der jeweils andere Extremwert gespeichert, entsteht eine eindimensionale AABB und beide Teilbäume können identisch behandelt werden. Dann kann für jeden
%GRR Subjekt "Jeden" oder "jeden Teilbaum"
 individuell ein Überlappungstest bestimmen, ob sich $\qb$ mit dem Teilbaum überschneidet. So können unter bestimmten Umständen sogar beide Teilbäume ausgeschlossen werden. Ein Beispiel dafür sieht man in Abb.~\ref{fig:boxsortNode}.\\
...
Man sieht in Abb.~\ref{fig:boxsortNode}, dass QB1 sich komplett links der Bereiche der beiden Teilbäume befindet. Der Suchlauf endet für QB1 somit direkt bei dem dargestellten Knoten
%GRR welchem der 4 dargestellten knoten?
. Auch QB2 überschneidet sich mit keinem Bereich der Teilbäume, befindet sich diesmal aber zwischen beiden. Solch eine Lage muss im Allgemeinen nicht zwangsläufig existieren, denn die Bereiche der beiden Teilbäume können auch überlappen.
%GRR zu QB1 wurde an dieser stelle eine Konsequenz der Lage der QB genannt
QB3 hingegen überlappt mit dem rechten Teilbaum. Für die Suche scheidet also nur der linke Teilbaum aus.
%GRR OK, hier frage ich mich nun, wie der Algorithmus denn jetzt aufgerufen wird.
% "Für die Suche scheidet also nur der linke Teilbaum aus." bezieht sich auf einen test mit allen dargestellten QBs oder nur mit einem ? Sollte es besser heißen : 
% "Für die Suche scheidet in diesem Fall also nur der linke Teilbaum aus." Das würde sich auf das Teilbeispiel mit QB3 beschränken?
 Diese wird im rechten Teilbaum rekursiv fortgesetzt. Man kann auch Query-Boxes konstruieren, sodass eine Überlappung mit beiden Teilbäumen auftritt. Dann muss die Suche in beiden rekursiv fortgesetzt werden.\\

%GRR new subtopic from here , new paragraph, more newlines, consider \subsubsection{•} 
 
Der eigentliche Sortiervorgang zum Aufbau des Baums wird mit einer Variante von Quicksort durchgeführt. Dabei wird das Pivot-Element zum Knoten und die restlichen Elemente werden entsprechend des Schlüssels in den beiden Teilbäumen zugeteilt.
%GRR Vorschlag :
%% Dabei wird ein Pivot-Element gewählt und zum Schlüssel eines Knotens erklärt, nachdem die übrigen Elemente dann binär in dessen Teilbäume einsortiert werden.
 Bei der Zuteilung wird nebenbei die 1D-AABB des Teilbaums ermittelt, indem die Ausdehnung bei jedem Zuteilungsschritt ggf. vergrößert wird, um das aktuelle Element zu berücksichtigen.\\
Als Heuristik für das Finden des Pivot-Elements wird der Median einer zufälligen Auswahl von bis zu 9 Elementen gewählt.
%GRR gibts da einen speziellen Grund konkret dafür, oder ist das einfach eine einfache, aber trotzdem ausreichende Lösung an dieser Stelle?
Die Sortierdimension wird an der Wurzel des gesamten Binärbaums auf die x-Achse festgelegt. Beim Einsortieren in die beiden Teilbäume wird eine Heuristik berechnet, die im jeweiligen Teilbaum die beste Sortierdimension abschätzt. Dafür wird für jede Dimension die durchschnittliche Größe der Objekte
%GRR aller Objekte oder nur objekte im Teilbaum ?
 in dieser Dimension berechnet. Damit kann man die Dichte der Objekte in jeder Dimension abschätzen, wenn man den Gesamtbereich kennt, den die AABBs einnehmen. Gewählt wird die niedrigste Dichte.\\ 
%GRR aus welcher Menge an Dichten? bist jetzt ist nur von einer Objektdichte die Rede
Im Gegensatz zum in Abschnitt~\ref{sec:spatialHashing}
%GRR spacial hashing kommt nachher. Für den sequenziellen leser unvorteilhaft und unverständliche Referenz, "was soll ich jetzt zuerst lesen ?", wenn auch prinzipiell durch cross-referenzing formell nicht falsch.
 beschriebenen Spatial Hashing müssen für den Boxsort-Algorithmus vor dem Start erst alle Objekte gesammelt werden. 
Im asymmetrischen Fall muss dann entschieden werden, aus welcher Menge der Objekte der Baum aufgebaut wird.
%GRR Vorschlag : 
% Im asymmetrischen Fall wird der Baum aus der Mastermenge $\obj_M$ aufgebaut, die aus einer Menge von 2 Mengen ausgewählt werden muss.
Für den Benutzer stehen hier mehrere Modi zur Verfügung. 
Zur Auswahl stehen 
Vergleiche der Anzahlen von Elementen in den Mengen.
%GRR nicht viel auswahl ... möglicherweise auswahl zwischen Vergleichsrichtungen?
% zweites element der auswahl kommt erst sehr viel später im Text. Bis dahin hab ich schon vergessen, dass das hier ne aufzählung ist. Suggestion : latex enumerate or itemize environment
 Der Grund dafür wird deutlich, wenn die Laufzeit im erwarteten Fall betrachtet wird : $mathcal{O}(|\obj_M|*log(|\obj_M|))$. Wenn eine ausreichende Verteilung im Raum vorliegt und die Größe von QB in der selben Größenordnung ist wie Elemente aus $\obj_M$ ist, dann benötigt die Suche $mathcal{O}(log(|\obj_M|))$ Zeit \cite[//TODO]{houthuys1987box}
 %GRR citation wrongly displayed and not in list
. 
Insgesamt wird also für das Finden aller Überlappungen $mathcal{O}(|\obj_M|*log(|\obj_M|)+|\obj_S|*log(|\obj_M|))$ Zeit benötigt. Anhand dieser Form steht die Vermutung nahe, dass die Wahl von $\obj_M$ immer auf die größere Menge fallen sollte, um das logarithmische Verhalten am gewinnbringendsten auszunutzen.
In der Praxis ist vor den Termen $|\obj_M|*log(|\obj_M|)$ und $|\obj_S|*log(|\obj_M|)$ aber ein Vorfaktor, der nicht identisch
%GRR nicht identisch zwischen welchen Szenarien?
 ist und deshalb auch beachtet werden muss. Messungen ergaben einen nahezu konstanten Vorfaktor beim Baumaufbau. Für den Abfrage-Durchlauf
%GRR ganz oben gibts ein TODO mit "Box-Sort einheitlich"
% Ich gehe mal davon aus, dass der Ablauf da definiert werden sollte. Dabei sollte dann auch definiert werden, was der Abfrage-Durchlauf eigentlich genau ist, und wann er ausgeführt wird.
  ist der Faktor jedoch stark abhängig von der konkreten Verteilung der eingegebenen AABBs. Messungen ergeben einen um den Faktor von ca.~3 geringeren Vorfaktor unter stark idealisierten Bedingungen
%GRR grammar ? meaning?
  , einen nahezu identischen unter moderaten Bedingungen und einen ca. 3-mal größeren unter erschwerten Bedingungen (Durchschnittliche Kollisionen pro Objekt>3). Den Benchmark-Ergebnissen in ~\ref{sec:benchmark}
  %GRR benchmark kommt später ... entweder hier einfügen, oder diesen Teil der Analyse, bzw. die Analyse erst dann machen, wenn alle für die Aussage relevanten Daten vorliegen und dem Leser bekannt sind. So kann hier auch Redundanz vorgebeugt werden.
   kann man entnehmen, dass wenn die beiden Mengen die gleiche Verteilung aufweisen die kleinere Menge meist die bessere Wahl ist, vor allem bei geringer Dichte der Objektverteilung. 
   Um trotzdem offen für zukünftige problemspezifische Erkenntnisse zu sein, bleibt die Option, die größere oder die kleinere Menge zu wählen. Dazu
%GRR Zusätzlich ? 
 kann ein Präferenz-Faktor angegeben werden, der vor der Entscheidung mit der Anzahl der Master-Elemente multipliziert wird.
 
 
Als weitere Modi kann der Benutzer einstellen, dass nur eine der beiden Mengen
%GRR konkretisiert am Objekttyp?
 für den Baum verwendet werden darf. Dies wird für Fälle gebraucht, wo die o.g. Bedingungen
 für die Komplexität der Suche im Baum nur für eine der beiden Mengen zutreffen.\\
 %GRR unverständlich : welche bedingung genau? verteilung? relative größe der mengen ? wie können diese Bedingungen nur auf eine der Mengen zutreffen?
...
Die Grundidee besteht hier dabei, den Raum in viele Bereiche
%GRR wie sind die Bereiche gearted, aufgeteilt? cubisch, gleichmäßig, gegenseitig überschneidungsfrei? etc.
% basically : define "Chunks", as this is first occurance of the term, a little more
% I know, you know, almost every minecraft player knows, professors don't typically
 (sog. Chunks) aufzuteilen und Kollisionspaare nur innerhalb dieser Teilbereiche zu suchen. Sind 2 Objekte
%GRR Objekte oder AABBs ? denke AABBs hier besser, Objekte haben Geschwindigkeit
% so könnte man geschickt umgehen, relative Geschwindigkeiten zu erklären....
  weit voneinander entfernt und somit nicht im selben Chunk, ist eine Überschneidung der AABBs nicht möglich und muss deshalb auch nicht betrachtet werden. Ein Beispiel für 2 Dimensionen sieht man in Abb.~\ref{fig:spatialHashing}.
...
%GRR curious question : wieso ist die konstante Maximalgröße wichtig, weil sonst die komplexität mit der maximialgröße des Objekts durch anmeldeaufwand steigt ?
Werden mehrere Objekte, die für eine Kollision kompatibel sind, in einem Chunk angemeldet, so müssen alle potentiellen Kollisionspaare überprüft werden (hier mit naivem //TODO Mathe
%GRR weis nicht ob nach meiner änderung dein TODO nicht schon erfüllt ist
 $\mathcal{O}(|\obj|^2)$ bzw. $\mathcal{O}(|\obj_M|*|\obj_S|)$ Algorithmus). Die Überprüfung kann entweder direkt bei der Anmeldung erfolgen (hier implementiert) oder erst nach Eintragung aller Objekte erfolgen, was die Auswahl an für die weitere Auflösung verwendbaren Algorithmen erhöhen würde.\\
...
%GRR todo
Der Idealfall an dem Spatial-Hashing asymptotisch optimal wird wurde bereits beschrieben.
%GRR wieso wird es dann hier nochmal erwähnt?
In der Praxis tritt dies jedoch nur selten auf. Problematisch können dort z.B. große Objekte
%GRR Wieder : Objekte oder AABBs ... das Objekt kann winzig sein, die AABB aber endsfett
 werden, denn die Anzahl Chunks, wo eine Anmeldung erforderlich ist, und damit die Laufzeit, steigt dann linear mit dem Volumen.
%GRR grammar ... meaning?
Ein weiterer Problemfall ist, wenn zu viele Objekte in einem Chunk sind. Denn selbst wenn sie sich nicht überschneiden, ist dann eine $\mathcal{O}(|\obj|^2)$ bzw. $\mathcal{O}(|\obj_M|*|\obj_S|)$ Auflösung erforderlich ($n$ und $m$ hier Anzahl Objekte im Chunk). Ob dieser Algorithmus für den Einsatz sinnvoll ist, hängt also davon ab, ob diese Probleme bei dem bestimmten Interaktionstyp auftreten, wo er eingesetzt werden soll.\\


\subsection{Vergleich der Verfahren}
\label{sec:benchmark}
Um die Performanz der Algorithmen bei verschiedenen Szenarien objektiv bewerten zu können, wurde im Rahmen des Projektes eine Benchmark-Umgebung implementiert. Getestet wurden verschiedene Szenarien und Parameter. Gemeinsam haben alle Szenarien die Testparameter Problemgröße (Objektzahl) und absolute Größe der Objekte (aus $\mathcal{S}$).\\
%GRR die absolute größe ist also ein spacelen, der wie zustande kommt? keine Vorstellung als Leser, wie eine für ein 3D-Objekt representative Größenvariable in ein 1D-Spacevec reinpasst
...
Ein weiteres rein asymmetrisches Szenario "Shotgun" modelliert eine Menge an Projektilen und Zielen. Die Ziele sind weiterhin gleichverteilt, die Projektile modellieren jedoch eine Schrotladung, bei der viele Projektile relativ nahe zusammen in eine sehr ähnliche Richtung fliegen. Dies bedeutet, dass die AABBs der Projektile sich weitgehend überlappen (AABBs werden über den gesamten Raum ausgedehnt, durch den sich Objekte im Laufe eines Ticks bewegen).
%GRR das wäre vielleicht praktisch, mal irgendwo am anfang zu erwähnen, dass AABBs eben nicht nur \supseteq K_0 sind, sondern \supseteq \{K_{o,t} | t\in \Upsilon_{\delta i}\} oder referenziert diesen fakt aus der section \ref{sec:bounding_volume}.
Sie werden aber alle nur in einem Teilbereich erzeugt, der nicht dem Erzeugungsbereich der Zielobjekte entspricht.
%GRR Üblicherweise nicht, ist aber nicht auszuschließen, talking of point blank range
Ein Testparameter beschreibt die erwartete Anzahl Ziele im Erzeugungsbereich der Projektile. Das Verhältnis zwischen der Menge der Zielobjekte und der Projektile ist ein weiterer Parameter.\\
...
Um die Probemenge zu erhöhen wurde ein Dummy-Parameter eingefügt, der den Seed ändert aber sonst keine Auswirkungen hat.
%GRR Dummy parameter implementierungsdetail? parameter existiert um Seed zu ändern and to what end würde genügen und wäre interessanter\\
...
Zunächst soll bei Boxsort die Wahl der Klasse
%GRR welche klassifizierung ist konkret gemeint, wsl. Interaktionstyp? Definitionen!
 für den Baum untersucht werden (in Abschnitt~\ref{sec:boxsort} theoretisch betrachtetes Problem). Zu diesem Zweck wird das Verhältnis der Ausführungszeiten beider Möglichkeiten
%GRR ...
 betrachtet. Ist es größer als 1, bedeutet dies hier, dass es mehr Ausführungszeit benötigt hat, die Master-Klasse
%GRR Wer Master ist und wer nicht ist u.U. interpretierungssache? Terminologie?
 für den Baum zu verwenden, d.h. die Slave-Klasse ist
%GRR praktisch
die bessere Wahl für den Baum.\\
...
%GRR Diagrammtyp Histogramm ?
    Einfluss der Wahl der Objektmenge für den Baumaufbau beim Box Sort, wenn gleich große Mengen
%GRR welche Mengen sind gleich groß?    
     getestet wurden, die beide statistisch gleichverteilt über den selben Raum sind.}
    \label{fig:boxsortCoice-reference}
\end{figure}

%GRR Unverständlich :  Objektanzahl, aber hier Asymmetrischer Fall? welche Objektanzahl ist gemeint
%GRR Unverständlich : Samples. Von was ? sind das einzelne Tests?
