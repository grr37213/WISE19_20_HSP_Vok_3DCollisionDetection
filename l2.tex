...
\ \\
		...
		\begin{itemize}
			\item [Linie] $L = \{l | x\in\mathfrak{F} ; l_0, l_1 \in \mathfrak{F}^3 ; l = l_0 + x * l_1; 0\le x\le 1 \}$ \\
			mit Startpunkt $l_0$ und Endpunkt $l_0 + l_1$
		\item [Dreieck] $T = \{t | x,y \in\mathfrak{F}; t_0, t_1, t_2 \in \mathfrak{F}; t = t_0 + x*t_1 + y*t_2; 0\le (x+y) \le 1\}$\\
			mit Eckpunkten $a = t_0 ; b = t_0 + t_1 ; c = t_0 + t_2$ 
			\item [Parallelogramm] $P = \{p | x,y \in\mathfrak{F}; p_0, p_1, p_2 \in \mathfrak{F}; p = p_0 + x*p_1 + y*p_2; 0\le x\le 1; 0\le y\le 1\}$\\
