...
\item Es wird ein Experiment der Vernachlässigung von Rotation durchgeführt. Es soll dabei beantwortet werden, ob auch ohne Behandlung der Rotation eine zufriedenstellende Illusion von physikalischem Realismus geschaffen werden kann.
\end{enumerate}
...
Es müssen nicht alle Merkmalskombinationen $\in \{Fläche, Kante, Ecke\}^2$ getestet werden. Es konnten essenzielle Merkmalskombinationen ermittelt werden, welche bei sich bei einer Erstkollision auftreten können:
		\begin{itemize}
			\item [$\{Ecke, Fläche\}$] Eine Ecke durchschlägt eine Fläche.\\
				$\Rightarrow$ Zu überpfüfende Paare: $(V_0\times A_1)\times (V_1\times A_0)$
			\item [$\{Kante, Kante\}$] Kanten durchschneiden sich gegenseitig.
				$\Rightarrow$ Zu überprüfende Paare: $(E_0\times E_1)\times (E_1\times E_0) = (E_0\times E_1)$
		\end{itemize}
		Alle anderen Kombinationen sind entweder in diesen beiden enthalten (z.B. $\{Ecke, Kante\}$ in 1.) oder eine Ereignis dieser beiden Szenarien muss logisch vorher passieren (z.B. 1. oder 2. muss vor $\{Kante, Fläche\}$ bereits passiert sein).
\ \\
		Aufgrund der Relativierung muss nur eines der Merkmale muss eine zeitliche Bewegung durchführen
		\begin{itemize}
			\item [$\{Ecke, Fläche\}$] 2 Möglichkeiten:
				\begin{itemize}
			...
			\item [$\{Kante, Kante\}$]  Parallelogramm(Linie \& Zeitdimension) schneidet Linie
		\end{itemize}
\ \\
...
\ \\
		...
		\begin{itemize}
			\item [$\{Ecke, Fläche\}$] $l_0 + x * l_1 = t_0 + y*t_1 + z*t_2$\\
				$x$ ist zudem hier der Koeffizient der Zeit, da die Linie in der Zeitdimension liegt.
			\item [$\{Kante, Kante\}$] $l_0 + x * l_1 = p = p_0 + y*p_1 + z*p_2$\\
...
\ \\
		Komplexität:
\begin{itemize}
			\item [$\{Ecke, Fläche\}$] $|V(O_0, t_0)|* |A(O_1, t_0)| + |V(O_1, t_0)|*|A(O_0, t_0)|$
			\item [$\{Kante, Kante\}$] $|E(O_0, t_0)| * |E(O_1, t_0)|$
		\end{itemize}
\ \\
