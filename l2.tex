...
Die Implementierung des GJK bezieht sich also auf die Suche nach dem nächsten, bzw. umschließenden Simplex zum Ursprung.\\
...
Das neue nächste gefundene Simplex wird $s$ zugewiesen $s \equiv \tilde{s} : \tilde{s},\hat{s} \in s_{near} : s_{near} = \{\hat{s} | distance(\hat{s}, O) = min\{distance(s', O) | s' \in s_{check}\}\}; |\tilde{s}| \leq |\hat{s}|$
