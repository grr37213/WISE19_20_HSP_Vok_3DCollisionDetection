...
\subsection{Input}
Aus dem vorhergehenden Schritt der Kollisionspaarermittlung werden für den Schritt der paarweisen Kollisionserkennung bis zu  $|O|^2$ Objektpaare erhalten ($O :=$ Menge simulierter Objekte), für die eine Kollisionsvermutung während eines Ticks gilt. Man geht weiter davon aus, dass die Menge der tatsächlichen Kollisionen $K_{definitive}\subseteq O^2$ in der Menge der vermuteten Kollisionspaare $K_{guess}\subseteq O^2$  vollständig enthalten ist $K_{definitive}\subseteq K_{guess}$, d.h. keine tatsächlichen Kollisionspaare im Paarermittlungsprozess verlorengehen.
...
\begin{enumerate}
	\item Inklusion\\
		Die Frage ob Objekte sich an einem bestimmten Zeitpunkt schneiden/gegenseitig enthalten. $O_0 \cap O_1 \neq \emptyset$.
	\item Intrusion\\
		...
		Hier beziehen wir uns auf die Zeitspanne eines Ticks $\delta_i$.
		Ermittelt wird, ob 
		$$\exists s \in [0, |\Upsilon_{\delta i}|], t_{nc} = \Upsilon_{\delta i}(s), t_c= \Upsilon_{\delta i}(s+1):$$
		$$  D(O_0, t_{nc})\cap D(O_1, t_{nc}) = \emptyset \wedge D(O_0, t_c)\cap D(O_1, t_c) \neq \emptyset$$
		und können dann aus der Schnittmenge $D(O_0, t_c)\cap D(O_1, t_c)$ Ort und beteiligte Teile des Objekts ermitteln.
\end{enumerate}
