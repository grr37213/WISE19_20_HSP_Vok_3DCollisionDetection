...
Ecken (eng. \textit{vertices}): $ V = \{s + (f * rotm(rot(O)) | s = pos(O), f\in m(O)\} = \{v_0, v_1 ... , v_{|V|-1}\}$\\
Kanten (eng. \textit{edges}): $ E = {(v_a, v_b), (v_b, v_c),(v_c, v_a) | (a, b, c) \in I\} $,\\
welche die Punkte $E_p:E\mapsto\mathbb{F}^3; E_p((v_a, v_b)) = \{v_a + (v_b-v_a)* k; k \in [0,1]\} $ im Raum einnehmen\\
Flächen (eng. \textit{areas}): $ A = \{(v_a, v_b, v_c) | (a, b, c) \in I\} $,\\
welche die Punkte $A_p:A\mapsto\mathbb{F}^3; A_p((v_a, v_b, v_c)) = \{v_a + (v_b-v_a)* k; k \in [0,1]\} $ im Raum einnehmen\\
...
Vertexpositionen zu bestimmten Zeiten können durch Matrixtransformationen errechnet werden.\\
